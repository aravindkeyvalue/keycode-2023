  ...
  public format = (value: any, type: DataType) => {
    switch (type) {
      ...
      case DataType.PHONE:
        ...
        const phoneNumber: PhoneNumber = this.phoneUtil.parseAndKeepRawInput(
          phone,
          "US"
        );
        // check if valid, otherwise return null
        if (!this.phoneUtil.isValidNumberForRegion(phoneNumber, "US")) {
          logger.info(`Phone ${phone} was invalid`);
          return null;
        } else {
          phone = this.phoneUtil.format(phoneNumber, PhoneNumberFormat.E164);
          logger.info(`Returning formatted phone ${phone}`);
          return phone;
        }
      case DataType.TIMESTAMP:
        const formattedTimestamp: number = moment.utc(value).valueOf(); // UNIX with mills
        return formattedTimestamp && !isNaN(formattedTimestamp)
          ? formattedTimestamp
          : null;
      default:
        logger.info(`No compatible type so just returning raw value`); // should be impossible with Enum
        return value;
    }
  }
